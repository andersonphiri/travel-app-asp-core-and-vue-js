        ...
        <v-list-item-group active-class="pink--text">
          <div v-if="packages && packages.length > 0">
           ...
           <template>
                 ...
                 <v-list-item-content>
                    <v-list-item-title v-text="tourPackage.name"></v-list-item-title>
                    ...
                    ></v-list-item-subtitle>
                    <v-list-item-subtitle
                        class="text--secondary" v-text=" `${tourPackage.instantConfirmation ? 'Instant Confirmation' : ''}`"
                    ></v-list-item-subtitle>
                    </div>
  </v-list-item-content>
  <v-list-item-action>
    <update-tour-package-form :bodyRequest="tourPackage" />
    <v-icon @click="removeTourPackage(tourPackage.id)"> mdi-delete-outline </v-icon>
  </v-list-item-action>
  ...
  components: { UpdateTourPackageForm,  },
    ...
    methods: {
        ...
        removeTourPackage(packageId) {
            const confirm = confirm("You really mean it, you wanna delete this package?");
            if (!confirm) return;
            // might need to wait for 1 minute because of cache
